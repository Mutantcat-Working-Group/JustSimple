...
public class HeaderTest extends HttpTester {
    ...
    public void testContentType_post_form() throws Exception {
        String rst = path("/demo2/header/ct").data("name", "justsimple").post();
        assert rst.equals("POST::application/x-www-form-urlencoded::justsimple");
    }

    @Test
    public void testContentType_post_multipart() throws Exception {
        String rst = path("/demo2/header/ct").data("name", "justsimple").multipart(true).post();
        assert rst.startsWith("POST::multipart/form-data");
        assert rst.endsWith("::justsimple");
        assert rst.equals("POST::multipart/form-data::justsimple") == false;


        rst = path("/demo2/header/ct?name=justsimple").get();
        assert rst.equals("GET::null::justsimple");
    }

    @Test
    public void testContentType_get() throws Exception {
        String rst = path("/demo2/header/ct?name=justsimple").get();
        Assertions.assertEquals("GET::null::justsimple", rst);
    }
    ...
    public void testServer_get() throws Exception {
        ...
        System.out.println(rst);
        Assertions.assertEquals("justsimple", rst);
    }

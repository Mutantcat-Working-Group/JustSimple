...
//        return SnEL.evalTmpl(tmpl, context);
    ...
    public <T> T toBean(String keyStarts, Class<T> clz) {
        if (JustSimple.app() != null) {
            JustSimple.context().aot().registerEntityType(clz, null);
        }
     ...
     * <p>
     * 别名冲突检测仅在调试模式下开启（{@link org.mutantcat.justsimple.JustSimpleProps#isDebugMode()})；
     * 批量 load 永远跳过检测。
     */
    @Override
    ...
    protected Object putDo(Object key, Object value, boolean checkConflict) {
        ...
        try {
            ...
            // 用 JustSimple.cfg() 短接，避免 appIf lambda 分配
            if (checkConflict && key instanceof String && value != null) {
                JustSimpleProps cfg = JustSimple.cfg();
                if (cfg != null && cfg.isDebugMode()) {
                    ...
                    if (alt != null) {
                        ...
                        if (old != null && !value.equals(old)) {
                            System.err.println("JustSimple-Warning: Props key conflict: '"

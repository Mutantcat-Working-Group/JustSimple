 ...
 * justsimple.threads.parallelPoolSize: 0
 * #调度线程池大小（默认：cpu*2，支持固定值 8 或内核倍数 x2）
 * justsimple.threads.scheduledPoolSize: 0
 * }</pre>
 *
 * @author noear
 * @since 1.12
...
public class RunHolder {
     ...
     * 并行线程池大小配置
     */
    private static final String EXECUTION_POOL_SIZE = "justsimple.task.execution.pool.size";
    /**
     * 调度线程池大小配置
     */
    private static final String SCHEDULING_POOL_SIZE = "justsimple.task.scheduling.pool.size";
    ...
    public ExecutorService getParallelExecutor() {
        if (parallelExecutor == null) {
            ...
            try {
                if (parallelExecutor == null) {
                    if (JustSimple.appIf(app -> app.cfg().isEnabledVirtualThreads())) {
                        parallelExecutor = ThreadsUtil.newVirtualThreadPerTaskExecutor("JustSimple-executor-");
                    } else {
                        int asyncPoolSize = getParallelPoolSize();
                        parallelExecutor = Executors.newFixedThreadPool(asyncPoolSize,
                                new NamedThreadFactory("JustSimple-executor-"));
                    }
                }
            } finally {
                Utils.locker().unlock();
                ...
                if (scheduledExecutor == null) {
                    if (JustSimple.appIf(app -> app.cfg().isEnabledVirtualThreads())) {
                        scheduledExecutor = Executors.newScheduledThreadPool(0,
                                ThreadsUtil.newVirtualThreadFactory("JustSimple-scheduledExecutor-"));
                    } else {
                        int scheduledPoolSize = getScheduledPoolSize();
                        scheduledExecutor = Executors.newScheduledThreadPool(scheduledPoolSize,
                                new NamedThreadFactory("JustSimple-scheduledExecutor-"));
                    }
                }
            } finally {
                Utils.locker().unlock();
     ...
     * <p>
     * 配置：justsimple.threads.parallelPoolSize，支持固定值 16 或内核倍数 x4
     */
     ...
     * 调度线程池大小（默认：cpu*2）
     * <p>
     * 配置：justsimple.threads.scheduledPoolSize，支持固定值 8 或内核倍数 x2
     */
    ...
    private int getPoolSizeOfConfig(String propName) {
        if (JustSimple.cfg() == null) {
            return 0;
        }

        //支持：16 或 x16（倍数）
        String poolSizeStr = JustSimple.cfg().get(propName);

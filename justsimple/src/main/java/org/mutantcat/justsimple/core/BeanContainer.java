...
import org.mutantcat.justsimple.annotation.Around;
import org.mutantcat.justsimple.annotation.Inject;
import org.mutantcat.justsimple.core.aspect.Interceptor;
import org.mutantcat.justsimple.core.aspect.InterceptorEntity;
import org.mutantcat.justsimple.core.exception.InjectionException;
import org.mutantcat.justsimple.core.runtime.AotCollector;
...
public abstract class BeanContainer {
    ...
    public BeanContainer(JustSimpleApp app, ClassLoader classLoader, Props props) {
        this.app = app;
    ...
    /**
     * 获取属性（写法上，更接近 JustSimple.cfg()）
     */
    public Props cfg() {
        if (props == null) {
            return JustSimple.cfg();
        } else {
            return props;
        }
    }
...
//        beanBaseSubscribersOfType.clear();
    ...
    private void beanInjectDo(VarHolder vh, String name, boolean required, boolean autoRefreshed) {
        if (Utils.isEmpty(name)) {
            ...
            if (JustSimpleApp.class.isAssignableFrom(vh.getType())) {
                vh.setValue(app());
                return;
            }

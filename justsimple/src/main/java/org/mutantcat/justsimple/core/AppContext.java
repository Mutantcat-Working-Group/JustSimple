...
import org.mutantcat.justsimple.core.convert.Converter;
import org.mutantcat.justsimple.core.convert.ConverterFactory;
...
public class AppContext extends BeanContainer {
    ...
    public AppContext(ClassLoader classLoader, Props props) {
        this(JustSimple.app(), classLoader, props);
    }

    public AppContext(JustSimpleApp app, ClassLoader classLoader, Props props) {
    ...
    public void beanDeliver(BeanWrap bw) {
        ...
        if (bw.raw() instanceof LifecycleBean) {
            ...
            int index = bw.index();
            if (index == 0) {
                index = org.mutantcat.justsimple.core.util.IndexUtil.buildLifecycleIndex(bw.rawClz());
            }
    ...
    private void startInjectReview(int sel) throws Throwable {
        ...
        if (gatherList.size() > 0) {
            for (InjectGather gather : gatherList) {
                org.mutantcat.justsimple.core.util.IndexUtil.buildGatherIndex(gather, gatherList);
            }

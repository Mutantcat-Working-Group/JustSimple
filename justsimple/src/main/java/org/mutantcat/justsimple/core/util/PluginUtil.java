...
public class PluginUtil {
    ...
    public static void scanPlugins(ClassLoader classLoader, Collection<String> excludeList, Consumer<PluginEntity> consumer) {
        ...
        Collection<String> nameList = ScanUtil.scan(classLoader, "META-INF/justsimple", n -> n.endsWith(".properties"));
        ...
        for (String name : nameList) {
            ...
            if (res == null) {
                ...
                log.warn("JustSimple plugin: name=" + name + ", resource is null");
            } else {
    ...
    public static void findPlugins(ClassLoader classLoader, Properties props, Collection<String> excludeList,Consumer<PluginEntity> consumer) {
        String pluginStr = props.getProperty("justsimple.plugin");

        if (Utils.isNotEmpty(pluginStr)) {
            String priorityStr = props.getProperty("justsimple.plugin.priority");
            int priority = 0;

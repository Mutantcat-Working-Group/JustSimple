 ...
 */
package org.mutantcat.justsimple.core.handle;
...
import org.mutantcat.justsimple.core.route.RoutingTable;
import org.mutantcat.justsimple.core.route.RoutingTableDefault;
import org.mutantcat.justsimple.core.util.PathUtil;
import org.mutantcat.justsimple.core.util.DataThrowable;
...
public abstract class Gateway extends HandlerAide implements Handler, Render {
    ...
    public void addBeans(Predicate<BeanWrap> where, boolean remoting) {
        JustSimple.context().lifecycle(() -> {
            JustSimple.context().beanForeach(bw -> {
                if (where.test(bw)) {
                    if (remoting) {
    ...
    public void add(Class<?> beanClz) {
        if (beanClz != null) {
            BeanWrap bw = JustSimple.context().wrapAndPut(beanClz);
    ...
    public void add(Class<?> beanClz, boolean remoting) {
        if (beanClz != null) {
            add(JustSimple.context().wrapAndPut(beanClz), remoting);
        }
    }
    ...
    public void add(String path, Class<?> beanClz, boolean remoting) {
        if (beanClz != null) {
            add(path, JustSimple.context().wrapAndPut(beanClz), remoting);
        }
    }
...
//        }
    ...
    protected void addDo(String path, MethodType method, int index, Handler handler) {
        ...
        if (allowPathMerging()) {
            ...
            mainRouting.add(path2, method, index, handler.version(), handler);
        } else {
            mainRouting.add(path, method, index, handler.version(), handler);
        }
    }

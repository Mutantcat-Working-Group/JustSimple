...
import org.mutantcat.justsimple.core.convert.ConverterManager;
import org.mutantcat.justsimple.core.event.EventListener;
import org.mutantcat.justsimple.core.event.*;
import org.mutantcat.justsimple.core.exception.StatusException;
import org.mutantcat.justsimple.core.handle.*;
import org.mutantcat.justsimple.core.route.Router;
import org.mutantcat.justsimple.core.route.RouterDefault;
import org.mutantcat.justsimple.core.route.RouterHandler;
import org.mutantcat.justsimple.core.runtime.NativeDetector;
import org.mutantcat.justsimple.core.serialize.SerializerManager;
 ...
 * @since 1.0
 */
public class JustSimpleApp {
    static final Logger log = LoggerFactory.getLogger(JustSimpleApp.class);

    private final JustSimpleProps _cfg; //属性配置
    ...
    protected JustSimpleApp(Class<?> source, MultiMap<String> args) throws Exception {
        //添加启动类检测
        if (source == null) {
        ...
        //初始化配置
        _cfg = new JustSimpleProps(this, args);
     ...
     * 启动
     */
    protected void startDo(ConsumerEx<JustSimpleApp> initialize) throws Throwable {
        //1.0.打印构造时的告警
    ...
    private void initAwait() throws Throwable {
        String addr = cfg().get("justsimple.start.ping");
     ...
     * 初始化（不能合在构建函数里）
     */
    private void init(ConsumerEx<JustSimpleApp> initialize) throws Throwable {
    ...
    private void run() throws Throwable {
        ...
        //3.加载渲染关系
        cfg().find("justsimple.view.mapping." , (k, v) -> {
            ...
            try {
                ...
                if (ex instanceof StatusException) {
                    ...
                    if (se.getCode() != 404 && se.getCode() != 405) {
                        log.warn("JustSimpleApp tryHandle failed, code=" + se.getCode(), ex);
                    }
                } else {
                    //推送异常事件 //todo: Action -> Gateway? -> RouterHandler -> Filter -> JustSimpleApp!
                    log.warn("JustSimpleApp tryHandle failed!", ex);
     ...
     * 订阅事件
     */
    public <T> JustSimpleApp onEvent(Class<T> type, EventListener<T> handler) {
     ...
     * 订阅事件
     */
    public <T> JustSimpleApp onEvent(Class<T> type, int index, EventListener<T> handler) {
     ...
     * 订阅异常状态
     */
    public JustSimpleApp onStatus(Integer code, Handler handler) {
     ...
     * 启用 Http 信号接入
     */
    public JustSimpleApp enableHttp(boolean enable) {
        _enableHttp = enable;
        return this;
    }
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableWebSocket(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableSocketD(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableTransaction(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableCaching(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableScanning(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableStaticfiles(boolean enable) {
     ...
     * @param enable 是否启用
     */
    public JustSimpleApp enableSessionState(boolean enable) {

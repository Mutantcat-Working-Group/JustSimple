...
import org.mutantcat.justsimple.core.PropsLoader;
...
public class Utils {
    ...
    public static boolean isProxyClass(Class<?> clz) {
        return clz.getName().contains("$$JustSimple");
    }
    ...
    public static @Nullable String appFolder() {
        ...
        if (_appFolder == null) {
            ...
            // 修复 justsimple 运行目录如果存在中文，会导致无法加载外部配置文件的问题【Issue 337】
            String encodedUri = JustSimple.location().getPath();
            String uri;
            try {
                // 使用UTF-8解码路径，解决中文乱码问题，理论上UTF-8总是支持的
                uri = URLDecoder.decode(encodedUri, JustSimple.encoding());

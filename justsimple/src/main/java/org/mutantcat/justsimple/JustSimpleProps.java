 ...
 * //
 * JustSimple.cfg()
 * JustSimple.cfg().isDebugMode()
 * JustSimple.cfg().isDriftMode()
 * JustSimple.cfg().get("water.logger")
 * JustSimple.cfg().getProp("db1")
 * }</pre>
 *
 * @author noear
 * @since 1.0
 * */
public final class JustSimpleProps extends Props {
    ...
    private boolean enabledVirtualThreads = false; //justsimple.threads.virtual.enable: true
    ...
    public JustSimpleProps(JustSimpleApp app, MultiMap<String> argx) throws Exception {
        ...
        //4.4.加载配置 justsimple.config.load //支持多文件（只支持内部，支持{env}）
        Map<String, String> loadKeyMap = new TreeMap<>();
        doFind("justsimple.config.load", (key, val) -> {
        ...
        //4.6.加载扩展配置 justsimple.config.add //支持多文件（支持内部或外部，支持{env}）
        addConfig(getArg("config.add"), sysPropOrg);//替代旧的 justsimple.config, 与 config.load 配对
        ...
        if (JavaUtil.JAVA_MAJOR_VERSION >= 21) {
            enabledVirtualThreads = getBool("justsimple.threads.virtual.enabled", false);
        }

        //9. 插件排除
        find("justsimple.plugin.exclude[", (k, v) -> {
    ...
    private void syncArgToSys(String name) {
        ...
        if (val != null) {
            //如果为空，尝试从属性配置取
            System.setProperty("justsimple." + name, val);
        }
    }
     ...
     */
    public JustSimpleProps loadEnv(String keyStarts) {
     ...
     * @param condition 条件
     */
    public JustSimpleProps loadEnv(Predicate<String> condition) {
        System.getenv().forEach((k, v) -> {
            if (condition.test(k)) {
     ...
     * 2.之后同时更新 system properties 和 justsimple cfg
     */
    ...
    public String appName() {
        return get("justsimple.app.name");
    }
    ...
    public String appGroup() {
        return get("justsimple.app.group");
    }
    ...
    public String appNamespace() {
        return get("justsimple.app.namespace");
    }
    ...
    public String appTitle() {
        return get("justsimple.app.title");
    }
    ...
    public String appVersion(){
        return get("justsimple.app.version","1.0.0");
    }
    ...
    public String appLicence(){
        return get("justsimple.app.licence", "");
    }
    ...
    public boolean appEnabled() {
        return getBool("justsimple.app.enabled", true);
    }
    ...
    public boolean stopSafe() {
        if(stopSafe == null){
            stopSafe = "1".equals(get("justsimple.stop.safe"));
        }
    ...
    public int stopDelay() {
        if(stopDelay == null){
            stopDelay = Integer.parseInt(get("justsimple.stop.delay", "10s").replace("s", ""));
        }

 ...
 */
package org.mutantcat.justsimple.serialization.protostuff;
...
import org.mutantcat.justsimple.serialization.AbstractBytesEntityConverter;
 ...
 * @since 3.6
 */
public class ProtostuffEntityConverter extends AbstractBytesEntityConverter<ProtostuffBytesSerializer> {
    public ProtostuffEntityConverter(ProtostuffBytesSerializer serializer) {
        super(serializer);
    }
    ...
    public String[] mappings() {
        return new String[]{SerializerNames.AT_PROTOBUF};
    }
    ...
    protected Object changeValue(Context ctx, ParamWrap pWrap, int pIdx, Class<?> pt, LazyReference bodyRef) throws Throwable {
        ...
        if (pWrap.spec().isRequiredBody()) {
            return getSerializer().deserializeFromBody(ctx, pWrap.getType());
        } else {
            return null;
        }
    }

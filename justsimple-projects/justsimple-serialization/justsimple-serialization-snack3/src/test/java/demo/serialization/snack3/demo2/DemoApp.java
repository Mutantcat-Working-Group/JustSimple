...
import org.mutantcat.justsimple.serialization.snack3.SnackStringSerializer;
...
public class DemoApp {
    public static void main(String[] args) {
        JustSimple.start(DemoApp.class, args, app -> {
            app.context().getBeanAsync(SnackStringSerializer.class, e -> {
                initMvcJsonCustom(e);
            });
        });
    }
    ...
    private static void initMvcJsonCustom(SnackStringSerializer factory) {
        //示例1：通过转换器，做简单类型的定制
        ...
        factory.addEncoder(Double.class, s -> String.valueOf(s));

        //示例2：通过编码器，做复杂类型的原生定制（基于框架原生接口）
        factory.addEncoder(Date.class, (data, node) -> node.val().setNumber(data.getTime()));

        factory.getSerializeConfig().removeFeatures(Feature.BrowserCompatible);

        //示例3：重置序列化特性（例，添加序列化null的特性）
        factory.getSerializeConfig().setFeatures(

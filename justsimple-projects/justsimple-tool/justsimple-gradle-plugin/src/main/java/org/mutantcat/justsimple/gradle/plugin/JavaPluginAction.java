...
import org.mutantcat.justsimple.gradle.tasks.bundling.JustSimpleJar;
...
final class JavaPluginAction implements PluginApplicationAction {
    ...
    public void execute(@NotNull Project project) {
        ...
        TaskProvider<JustSimpleJar> justsimpleJar = configureJustSimpleJarTask(project, resolveMainClassName);
        configureArtifactPublication(justsimpleJar);
    ...
    private TaskProvider<ResolveMainClassName> configureResolveMainClassNameTask(Project project) {
        return project.getTasks().register(JustSimplePlugin.RESOLVE_MAIN_CLASS_NAME_TASK_NAME,
                ResolveMainClassName.class, (resolveMainClassName) -> {
                    ...
                    resolveMainClassName.getConfiguredMainClassName().convention(project.provider(() -> {
                        ...
                        JustSimpleExtension justsimpleExtension = project.getExtensions()
                                .findByType(JustSimpleExtension.class);

                        return Objects.requireNonNull(justsimpleExtension).getMainClass().getOrNull();
                    }));
    ...
    private void configureArtifactPublication(TaskProvider<JustSimpleJar> bootJar) {
    ...
    private TaskProvider<JustSimpleJar> configureJustSimpleJarTask(Project project, TaskProvider<ResolveMainClassName> resolveMainClassName) {
        ...
        return project.getTasks().register(JustSimplePlugin.JUSTSIMPLE_JAR_TASK_NAME, JustSimpleJar.class, (bootJar) -> {

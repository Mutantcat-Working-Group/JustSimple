 ...
 */
package org.mutantcat.justsimple.server.undertow;
...
import org.mutantcat.justsimple.server.ServerLifecycle;
import org.mutantcat.justsimple.server.ServerProps;
import org.mutantcat.justsimple.server.prop.impl.HttpServerProps;
import org.mutantcat.justsimple.server.undertow.http.UtHttpContextServletHandler;
import org.mutantcat.justsimple.server.undertow.websocket.UtWsProtocolHandshakeHandler;
...
public class UndertowServer extends UndertowServerBase implements ServerLifecycle {
    ...
    protected void setup(String host, int port) throws Throwable {
        ...
        if (ServerProps.request_maxBodySize > 0) {
            builder.setServerOption(UndertowOptions.MAX_ENTITY_SIZE, ServerProps.request_maxFileRequestSize());
        }

        if (ServerProps.request_maxFileSize > 0) {
            builder.setServerOption(UndertowOptions.MULTIPART_MAX_ENTITY_SIZE, ServerProps.request_maxFileRequestSize());
        }

        if (ServerProps.request_maxPartCount > 0) {
            builder.setServerOption(UndertowOptions.MAX_PARAMETERS, ServerProps.request_maxPartCount);
        }

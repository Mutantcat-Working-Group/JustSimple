...
import org.mutantcat.justsimple.server.netahttp.http.NetaHttpContext;
...
public class NetaHttpServerImpl implements ServerLifecycle {
    ...
    private void handleDo(NetaHttpContext ctx) {
        try {
            if (org.mutantcat.justsimple.JustSimple.app() != null && ServerProps.output_meta) {
                ctx.headerSet("JustSimple-Server", "neta-http/" + org.mutantcat.justsimple.JustSimple.version());
            }
            handler.handle(ctx);
        } catch (Throwable e) {

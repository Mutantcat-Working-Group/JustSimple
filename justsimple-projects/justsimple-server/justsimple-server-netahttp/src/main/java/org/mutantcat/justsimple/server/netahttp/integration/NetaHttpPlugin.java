 ...
 */
package org.mutantcat.justsimple.server.netahttp.integration;
...
import org.mutantcat.justsimple.server.netahttp.NetaHttpServerComb;
 ...
 * @since 3.10
 */
public final class NetaHttpPlugin implements Plugin {
    static final Logger log = LoggerFactory.getLogger(NetaHttpPlugin.class);
    ...
    public static String justsimple_server_ver() {
        return "neta-http 1.2/" + JustSimple.version();
    }

    private NetaHttpServerComb _server;
    ...
    private void start0(AppContext context) throws Throwable {
        ...
        if (props.isIoBound()) {
            // 如果是 io 密集型的，加二段线程池
            _server.setExecutor(props.newWorkExecutor("netahttp-"));
        }
        ...
        String connectorInfo = "justsimple.connector:main: netahttp: Started ServerConnector@{HTTP/1.1,[http/1.1]}";
        ...
        log.info("Server:main: netahttp: Started (" + justsimple_server_ver() + ") @" + (time_end - time_start) + "ms");
    }
    ...
    public void stop() throws Throwable {
        if (_server != null) {
            ...
            log.info("Server:main: netahttp: Has Stopped (" + justsimple_server_ver() + ")");
        }
    }
}

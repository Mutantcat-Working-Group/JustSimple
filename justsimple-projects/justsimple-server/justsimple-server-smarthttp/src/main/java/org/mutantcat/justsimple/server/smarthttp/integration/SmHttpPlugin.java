 ...
 */
package org.mutantcat.justsimple.server.smarthttp.integration;
...
import org.mutantcat.justsimple.server.smarthttp.SmHttpServerComb;
import org.mutantcat.justsimple.server.smarthttp.http.MultipartUtil;
...
public final class SmHttpPlugin implements Plugin {
    ...
    public static String justsimple_server_ver() {
        return "smart http 2.5/" + JustSimple.version();
    }
    ...
    public void start(AppContext context) throws Throwable {
        ...
        //如果有 jetty 插件，就不启动了
        if (ClassUtil.loadClass("org.mutantcat.justsimple.server.jetty.integration.JettyPlugin") != null) {
            return;
        }

        //如果有 undrtow 插件，就不启动了
        if (ClassUtil.loadClass("org.mutantcat.justsimple.server.undertow.integration.UndertowPlugin") != null) {
            return;
        }

        //如果有 tomcat 插件，就不启动了
        if (ClassUtil.loadClass("org.mutantcat.justsimple.server.tomcat.integration.TomcatPlugin") != null) {
            return;
        }

        //如果有 vertx 插件，就不启动了
        if (ClassUtil.loadClass("org.mutantcat.justsimple.server.vertx.integration.VxHttpPlugin") != null) {
            return;
        }

        //如果有 grizzly 插件，就不启动了
        if (ClassUtil.loadClass("org.mutantcat.justsimple.server.grizzly.integration.GyHttpPlugin") != null) {
            return;
        }
    ...
    private void start0(AppContext context) throws Throwable {
        ...
        String connectorInfo = "justsimple.connector:main: smarthttp: Started ServerConnector@{HTTP/1.1,[http/1.1]";
        ...
        log.info("Server:main: smarthttp: Started (" + justsimple_server_ver() + ") @" + (time_end - time_start) + "ms");
    }
    ...
    public void stop() throws Throwable {
        if (_server != null) {
            ...
            log.info("Server:main: smarthttp: Has Stopped (" + justsimple_server_ver() + ")");
        }
    }
}
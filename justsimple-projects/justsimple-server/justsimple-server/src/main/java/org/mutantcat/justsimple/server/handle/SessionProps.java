 ...
 */
package org.mutantcat.justsimple.server.handle;
...
public class SessionProps {
    ...
    static {
        session_timeout = JustSimple.cfg().getInt("server.session.timeout", 60 * 60 * 2);

        session_cookieName = JustSimple.cfg().get("server.session.cookieName", "JUSTSIMPLEID");

        session_cookieHttpOnly = JustSimple.cfg().getBool("server.session.cookieHttpOnly", true);

        //
        // cookieDomain
        //
        String tmp = JustSimple.cfg().get("server.session.cookieDomain");
        ...
        // cookieDomainAuto
        //
        tmp = JustSimple.cfg().get("server.session.cookieDomainAuto");

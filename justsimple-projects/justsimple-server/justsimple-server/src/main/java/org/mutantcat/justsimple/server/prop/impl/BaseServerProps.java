 ...
 */
package org.mutantcat.justsimple.server.prop.impl;

import org.mutantcat.justsimple.JustSimple;
import org.mutantcat.justsimple.Utils;
import org.mutantcat.justsimple.server.prop.ServerExecutorProps;
import org.mutantcat.justsimple.server.prop.ServerSignalProps;
...
public abstract class BaseServerProps implements ServerSignalProps, ServerExecutorProps {
    ...
    private void initSignalProps(int portBase) {
        name = JustSimple.cfg().get(PROP_NAME);
        port = JustSimple.cfg().getInt(PROP_PORT, 0);
        host = JustSimple.cfg().get(PROP_HOST);

        wrapPort = JustSimple.cfg().getInt(PROP_WRAP_PORT, 0);
        wrapHost = JustSimple.cfg().get(PROP_WRAP_HOST);

        //host + port
        if (port < 1) {
            port = portBase + JustSimple.cfg().serverPort();
        }

        if (Utils.isEmpty(host)) {
            host = JustSimple.cfg().serverHost();
        }

        //imageHost + imagePort
        if (wrapPort < 1) {
            wrapPort = JustSimple.cfg().serverWrapPort(true);
        ...
        if (Utils.isEmpty(wrapHost)) {
            wrapHost = JustSimple.cfg().serverWrapHost(true);
    ...
    private void initExecutorProps() {
        ioBound = JustSimple.cfg().getBool(PROP_IO_BOUND, true);
        idleTimeout = JustSimple.cfg().getLong(PROP_IDLE_TIMEOUT, 0L);

        //支持：16 或 x16（倍数）
        String coreThreadsStr = JustSimple.cfg().get(PROP_CORE_THREADS);
        ...
        //支持：16 或 x16（倍数）
        String maxThreadsStr = JustSimple.cfg().get(PROP_MAX_THREADS);

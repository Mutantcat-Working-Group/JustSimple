 ...
 */
package org.mutantcat.justsimple.config.yaml;
...
public class PropertiesYaml extends Properties {
    static final String JUSTSIMPLE_ENV = "justsimple.env";
    static final String JUSTSIMPLE_ENV_ON = "justsimple.env.on";
    static final String YML_PART_SPLIT = "---";

    AtomicReference<String> envRef = new AtomicReference<>(System.getProperty(JUSTSIMPLE_ENV));
    ...
    public void loadYml(Reader reader) throws IOException {
        ...
        for (int i = 0; i < partList.size(); i++) {
            ...
            this.loadYml(partStr, partProp -> {
                //2.同步环境变量
                if (envRef.get() == null && fi == 0 && partProp.containsKey(JUSTSIMPLE_ENV)) {
                    envRef.set(String.valueOf(partProp.get(JUSTSIMPLE_ENV)));
                }
            });
        }
    }
    ...
    public void loadYml(String str, Consumer<Map<Object, Object>> c) {
        ...
        //3.根据条件过滤加载
        if (partProp.containsKey(JUSTSIMPLE_ENV_ON)) {
            //如果有环境条件，尝试匹配 //支持多环境匹配。例：justsimple.env.on: pro1 | pro2
            String envOn = String.valueOf(partProp.get(JUSTSIMPLE_ENV_ON));

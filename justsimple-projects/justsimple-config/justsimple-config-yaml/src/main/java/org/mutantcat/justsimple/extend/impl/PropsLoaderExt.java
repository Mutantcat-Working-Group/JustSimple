...
import org.mutantcat.justsimple.config.yaml.PropertiesYaml;
...
/**
 * 属性加载器-静态扩展实现（静态扩展约定：org.mutantcat.justsimple.extend.impl.XxxxExt）
 *
 * @author noear
 * @since 1.5
 */
public class PropsLoaderExt extends PropsLoader {
    ...
    public Properties load(URL url) throws IOException {
        ...
        if (fileName.endsWith(".properties")) {
            if (JustSimple.appIf(app -> app.cfg().isDebugMode())) {
                log.trace(fileName);
            }
            ...
            try (InputStreamReader reader = new InputStreamReader(urlConn.getInputStream(), JustSimple.encoding())) {
                tmp.load(reader);
            }

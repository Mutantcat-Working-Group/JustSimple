...
import org.mutantcat.justsimple.core.FactoryManager;
 ...
 * @since 1.11
 */
public class JustSimpleMDCAdapter implements MDCAdapter {
    //改为懒加载
    ...
    private static ThreadLocal<Map<String, String>> threadMap() {
        if (_threadMap == null) {
            ...
            try {
                if (_threadMap == null) {
                    _threadMap = FactoryManager.getGlobal().newThreadLocal(JustSimpleMDCAdapter.class, false);
                }
            } finally {
                Utils.locker().unlock();
            }
    ...
    private static ThreadLocal<Map<String, Deque<String>>> threadMapOfStacks() {
        if (_threadMapOfStacks == null) {
            ...
            try {
                if (_threadMapOfStacks == null) {
                    _threadMapOfStacks = FactoryManager.getGlobal().newThreadLocal(JustSimpleMDCAdapter.class, false);
                }
            } finally {
                Utils.locker().unlock();
            }

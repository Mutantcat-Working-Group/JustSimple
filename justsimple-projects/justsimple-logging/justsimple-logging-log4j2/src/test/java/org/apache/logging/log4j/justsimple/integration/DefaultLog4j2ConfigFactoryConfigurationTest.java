...
public class DefaultLog4j2ConfigFactoryConfigurationTest {
    ...
    static final String[] PROPS = {
            ...
            "justsimple.logging.appender.console.pattern",
            "justsimple.logging.appender.console.level",
            "justsimple.logging.appender.file.pattern",
    ...
    @BeforeAll
    public static void bootJustSimple() {
        // createConfiguration 依赖 JustSimple.cfg()，需 JustSimple 已启动（同 JVM 中其它 @JustSimpleTest 已启动则跳过）
        if (JustSimple.app() == null) {
            JustSimple.start(DefaultLog4j2ConfigFactoryConfigurationTest.class, new String[]{});
        }
    }
    ...
    private LoggerContext startFactoryContext(boolean fileEnable, boolean consoleEnable) {
        ...
        JustSimple.cfg().put("justsimple.logging.appender.file.enable", String.valueOf(fileEnable));
        JustSimple.cfg().put("justsimple.logging.appender.console.enable", String.valueOf(consoleEnable));
    ...
    public void test_default_all_appenders() {
        // 默认（不设任何属性）：Console + File + JustSimple 三个 appender，root 全部引用
        System.setProperty("justsimple.logging.appender.file.name", new File(dir, "app").getPath());
        LoggerContext ctx = startFactoryContext(true, true);

        assertEquals(Arrays.asList("Console", "File", "JustSimple"), appenderNames(ctx));
        assertEquals(Arrays.asList("Console", "File", "JustSimple"), rootRefs(ctx));
    ...
    public void test_console_disabled() {
        System.setProperty("justsimple.logging.appender.console.enable", "false");
        System.setProperty("justsimple.logging.appender.file.name", new File(dir, "app").getPath());
        LoggerContext ctx = startFactoryContext(true, false);

        assertEquals(Arrays.asList("File", "JustSimple"), appenderNames(ctx));
        assertEquals(Arrays.asList("File", "JustSimple"), rootRefs(ctx));
    }

    @Test
    public void test_file_disabled() {
        System.setProperty("justsimple.logging.appender.file.enable", "false");
        LoggerContext ctx = startFactoryContext(false, true);

        assertEquals(Arrays.asList("Console", "JustSimple"), appenderNames(ctx));
        assertEquals(Arrays.asList("Console", "JustSimple"), rootRefs(ctx));
        assertFalse(new File(dir, "app.log").exists(), "file 关闭时不应产生日志文件");
    }

    @Test
    public void test_both_disabled() {
        // 双关闭：仅剩 JustSimple appender，root 只引用 JustSimple（极端但合法的配置）
        System.setProperty("justsimple.logging.appender.console.enable", "false");
        System.setProperty("justsimple.logging.appender.file.enable", "false");
        LoggerContext ctx = startFactoryContext(false, false);

        assertEquals(Arrays.asList("JustSimple"), appenderNames(ctx));
        assertEquals(Arrays.asList("JustSimple"), rootRefs(ctx));
    }
    ...
    public void test_totalSize_cap_via_factory_end_to_end() throws Exception {
        ...
        System.setProperty("justsimple.logging.appender.file.name", new File(dir, "app").getPath());
        System.setProperty("justsimple.logging.appender.file.maxFileSize", "1KB");
        System.setProperty("justsimple.logging.appender.file.totalSizeCap", "3KB");

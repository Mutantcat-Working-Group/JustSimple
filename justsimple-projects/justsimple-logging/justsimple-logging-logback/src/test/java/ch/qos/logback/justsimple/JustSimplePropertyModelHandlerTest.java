 ...
 */
package ch.qos.logback.justsimple;
...
/**
 * JustSimplePropertyModelHandler 场景测试：
 * name/source 缺失报错、取值（配置命中 / 默认值 / 无点号 source / 回退重查）
 *
 * @since 3.9.2
 */
public class JustSimplePropertyModelHandlerTest {

    static final String KEY_SET = "justsimple.test.prop.a.b.c";
    static final String KEY_UNSET = "justsimple.test.prop.never.set.key";

    LoggerContext loggerContext;
    JustSimplePropertyModelHandler handler;

    @BeforeAll
    public static void bootJustSimple() {
        if (JustSimple.app() == null) {
            JustSimple.start(JustSimplePropertyModelHandlerTest.class, new String[]{});
        }
    }
    ...
    public void setUp() {
        ...
        handler = new JustSimplePropertyModelHandler(loggerContext);
    }
    ...
    private JustSimplePropertyModel model(String name, String source, String defaultValue) {
        JustSimplePropertyModel m = new JustSimplePropertyModel();
        m.setName(name);
        m.setSource(source);
    ...
    @Test
    public void test_value_from_justsimple_config() throws Exception {
        JustSimple.cfg().put(KEY_SET, "value-from-cfg");
    ...
    public void test_source_without_dot_returns_default_directly() throws Exception {
        ModelInterpretationContext ic = intercon();
        handler.handle(ic, model("MY_PROP", "justsimpletestnodot", "plain-default"));
    ...
    public void test_scope_local_vs_context() throws Exception {
        JustSimple.cfg().put("justsimple.test.prop.scoped", "scoped-value");

        ModelInterpretationContext ic = intercon();
        JustSimplePropertyModel m = model("MY_PROP", "justsimple.test.prop.scoped", "dft");
        m.setScope("context");
        handler.handle(ic, m);

...
import ch.qos.logback.justsimple.JustSimpleAppender;
import ch.qos.logback.justsimple.JustSimpleTagsConverter;
...
public class DefaultLogbackConfiguration {
    void apply(LogbackConfigurator config) {
        ...
        try {
            ...
            Appender<ILoggingEvent> justsimpleAppender = justsimpleAppender(config);
            appenderList.add(justsimpleAppender);
    ...
    private void prepare(LogbackConfigurator config) {
        config.conversionRule("tags", JustSimpleTagsConverter.class);


        putProperty(config, "APP_NAME", "justsimple.app.name",
                "justsimple");

        putProperty(config, "CONSOLE_LOG_PATTERN", "justsimple.logging.appender.console.pattern",
                "%highlight(%-5level %d{yyyy-MM-dd HH:mm:ss.SSS} #${PID:-} [-%t][*%X{traceId}]%tags[%logger{20}]:) %n%msg%n");
        putProperty(config, "CONSOLE_LOG_LEVEL", "justsimple.logging.appender.console.level",
                "TRACE");

        putProperty(config, "FILE_LOG_EXTENSION", "justsimple.logging.appender.file.extension",
                ".log");
        putProperty(config, "FILE_LOG_NAME", "justsimple.logging.appender.file.name",
                "logs/${APP_NAME}");
        putProperty(config, "FILE_LOG_ROLLING", "justsimple.logging.appender.file.rolling",
                "${FILE_LOG_NAME}_%d{yyyy-MM-dd}_%i${FILE_LOG_EXTENSION}");
        putProperty(config, "FILE_LOG_PATTERN", "justsimple.logging.appender.file.pattern",
                "%-5level %d{yyyy-MM-dd HH:mm:ss.SSS} #${PID:-} [-%t][*%X{traceId}]%tags[%logger{20}]: %n%msg%n");
        putProperty(config, "FILE_LOG_LEVEL", "justsimple.logging.appender.file.level",
                "INFO");
        putProperty(config, "FILE_LOG_MAX_HISTORY", "justsimple.logging.appender.file.maxHistory",
                "7");
        putProperty(config, "FILE_LOG_MAX_FILE_SIZE", "justsimple.logging.appender.file.maxFileSize",
                "10 MB");

        putProperty(config, "FILE_LOG_TOTAL_SIZE_CAP", "justsimple.logging.appender.file.totalSizeCap",
                "0");


        putProperty(config, "LOGGER_ROOT_LEVEL", "justsimple.logging.logger.root.level",
                "TRACE");
    }

    private void putProperty(LogbackConfigurator config, String name, String source, String defaultValue) {
        config.getContext()
                .putProperty(name, resolveJustSimpleProp(config, source, defaultValue));
    }

    private Appender<ILoggingEvent> justsimpleAppender(LogbackConfigurator config) {
        JustSimpleAppender appender = new JustSimpleAppender();
        ThresholdFilter filter = new ThresholdFilter();
        filter.setLevel(resolve(config, "TRACE"));
        filter.start();
        appender.addFilter(filter);
        config.appender("JUSTSIMPLE", appender);
        return appender;
    }
    ...
    private String resolveJustSimpleProp(LogbackConfigurator config, String source, String defaultValue) {
        String val = JustSimple.cfg().getProperty(source);
        if (val == null) {
            val = defaultValue;
        }

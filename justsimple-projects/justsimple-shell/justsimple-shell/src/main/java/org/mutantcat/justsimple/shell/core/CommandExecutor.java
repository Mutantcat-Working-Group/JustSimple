package org.mutantcat.justsimple.shell.core;

import org.mutantcat.justsimple.i18n.I18nUtil;
import org.mutantcat.justsimple.shell.integration.ShellContext;
...
/**
 * 命令执行器，解析输入的shell命令，反射执行对应的方法（适配 JustSimple 原生 I18nUtil）
 *
 * @author noear
 * @author shenmk
 * @since 3.9.1
...
public class CommandExecutor {
    ...
    public static String execute(String commandLine) {
        // 1. 空命令校验（适配 JustSimple 原生 I18nUtil）
        ...
        // 3. 获取命令元数据（适配 JustSimple 原生 I18nUtil + 占位符替换）
        ...
        // 4. 校验必选参数数量（适配 JustSimple 原生 I18nUtil + 占位符替换）
        ...
        // 可选：参数数量上限提示（适配 JustSimple 原生 I18nUtil + 占位符替换）
        ...
        for (int i = 0; i < paramMetaList.size(); i++) {
            ...
            // 类型转换（适配 JustSimple 原生 I18nUtil + 占位符替换）
            try {
        ...
        // 6. 反射调用目标方法（适配 JustSimple 原生 I18nUtil + 占位符替换）
        try {
            Method targetMethod = metadata.getTargetMethod();
            // 可选：参数数量校验（适配 JustSimple 原生 I18nUtil）
            ...
            // 执行成功文案（适配 JustSimple 原生 I18nUtil）
    ...
    /**
     * 补充占位符替换逻辑（适配 JustSimple 原生 I18nUtil 不支持 {} 替换的问题）
     *
    ...
    /**
     * 参数类型转换（仅支持基础类型，适配 JustSimple 原生 I18nUtil）
     */

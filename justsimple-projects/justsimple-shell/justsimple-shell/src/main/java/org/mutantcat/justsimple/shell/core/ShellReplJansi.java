...
/**
 * JustSimple Shell 对外启动类，集成 JLine + Jansi 增强终端交互（极简默认配置版）
 *
 * @author shenmk
 * @author ZhangSan
 * @since 3.9.1
...
public class ShellReplJansi {
    ...
    public static void start() {
        ...
        try {
            ...
            // 彩色欢迎信息（保留核心文案，无额外配置）
            System.out.println(Ansi.ansi().fg(Ansi.Color.CYAN).a(getI18nMessage("shell.welcome", "欢迎使用 JustSimple Shell 终端")).reset());
            ...
            // 彩色提示符（默认样式）
            String prompt = Ansi.ansi().fg(Ansi.Color.BLUE).bold().a(getI18nMessage("shell.prompt", "justsimple> ")).reset().toString();

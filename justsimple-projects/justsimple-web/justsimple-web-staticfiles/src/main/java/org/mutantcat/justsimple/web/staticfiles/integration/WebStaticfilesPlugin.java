 ...
 */
package org.mutantcat.justsimple.web.staticfiles.integration;
...
import org.mutantcat.justsimple.web.staticfiles.*;
...
public class WebStaticfilesPlugin implements Plugin {
    ...
    public void start(AppContext context) {
        ...
        //2.添加映射
        List<Map> mapList = JustSimple.cfg().toBean(StaticConfig.PROP_MAPPINGS, ArrayList.class);
        if (mapList != null) {
            for (Map map : mapList) {
        ...
        //3.加载自定义 mime
        context.cfg().find("justsimple.mime.", (key, val) -> {

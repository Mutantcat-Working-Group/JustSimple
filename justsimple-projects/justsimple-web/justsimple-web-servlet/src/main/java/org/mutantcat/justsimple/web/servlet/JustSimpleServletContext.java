...
import org.mutantcat.justsimple.server.handle.AsyncContextState;
import org.mutantcat.justsimple.server.handle.ContextBase;
import org.mutantcat.justsimple.server.handle.HeaderNames;
import org.mutantcat.justsimple.server.io.LimitedInputStream;
import org.mutantcat.justsimple.server.util.DecodeUtils;
import org.mutantcat.justsimple.server.util.RedirectUtils;
 ...
 * @since 2.9
 * */
public class JustSimpleServletContext extends ContextBase {
    static final Logger log = LoggerFactory.getLogger(JustSimpleServletContext.class);

    private final HttpServletRequest _request;
    private final HttpServletResponse _response;
    private final boolean _useLimitStream;

    public JustSimpleServletContext(HttpServletRequest request, HttpServletResponse response) {
        this(request, response, false);
    }

    public JustSimpleServletContext(HttpServletRequest request, HttpServletResponse response, boolean useLimitStream) {
        _request = request;
        _response = response;
        _useLimitStream = useLimitStream;

        if (sessionState().replaceable()) {
            sessionState = new JustSimpleServletSessionState(request);
        }
    }
    ...
    @Override
    public void cookieSet(org.mutantcat.justsimple.core.handle.Cookie cookie) {

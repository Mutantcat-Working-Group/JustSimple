 ...
 */
package org.mutantcat.justsimple.web.servlet;
...
import org.mutantcat.justsimple.core.event.AppLoadEndEvent;
import org.mutantcat.justsimple.core.event.EventBus;
import org.mutantcat.justsimple.core.handle.ContextPathFilter;
 ...
 * @since 1.10
 */
public class JustSimpleServletContextListener implements ServletContextListener {
    /**
     * 对接 web.xml
     */
    @Override
    public void contextInitialized(ServletContextEvent sce) {
        ...
        EventBus.subscribe(AppInitEndEvent.class, e -> {
            ...
            JustSimple.app().enableHttp(false);

            //取消文件运行模式
            JustSimple.cfg().isFilesMode(false);

            //设定 contextPath
            String contextPath = sce.getServletContext().getContextPath();
            if (contextPath.length() > 1) {
                JustSimple.app().router().filterIfAbsent(Constants.FT_IDX_CONTEXT_PATH, new ContextPathFilter(contextPath, false));
            }
        });

        //2.注册加载完成事件
        EventBus.subscribe(AppLoadEndEvent.class, e -> {
            ServletRegistration.Dynamic registration = sce.getServletContext().addServlet("justsimple", JustSimpleServletHandler.class);
            if (registration != null) {
                //mapping
                registration.addMapping("/*");

                String _tempdir = IoUtil.getTempDirAsString("justsimple-server");
            ...
            if(e.app().enableWebSocket()){
                JustSimpleWebSocketEndpoint.addEndpoint(sce.getServletContext());
            }
        });
    ...
    private void invokeMain(ServletContext sc, String[] strArgs) throws RuntimeException {
        ...
        String mainClassStr = sc.getInitParameter("justsimpleMainClass");//v2.5
        if (Utils.isEmpty(mainClassStr)) {
            mainClassStr = sc.getInitParameter("justsimpleStartClass");//@deprecated v2.5
        }
    ...
    /**
     * Servlet容器销毁时关闭JustSimple
     */
    @Override
    public void contextDestroyed(ServletContextEvent sce) {
        if (JustSimple.cfg().stopSafe()) {
            JustSimple.stopBlock(false, JustSimple.cfg().stopDelay());
        } else {
            JustSimple.stopBlock(false, 0);
        }
    }
}

...
/**
 * Servlet Filter，适配为 JustSimple 入口
 *
 * @author noear
 * @since 1.2
 * */
public class JustSimpleServletFilter implements Filter {
    static final Logger log = LoggerFactory.getLogger(JustSimpleServletFilter.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        if (request instanceof HttpServletRequest) {
            ...
            //
            Context.currentWith(new JustSimpleServletContext((HttpServletRequest) request, (HttpServletResponse) response), ctx -> {
                try {
                    //过滤开始
                    doFilterStart(ctx);

                    //JustSimple处理(可能是空处理)
                    JustSimple.app().tryHandle(ctx);

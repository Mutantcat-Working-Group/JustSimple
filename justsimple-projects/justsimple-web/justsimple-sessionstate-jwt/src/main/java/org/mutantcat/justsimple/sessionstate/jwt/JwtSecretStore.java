...
/**
 * Stores generated default secrets in the JustSimple settings file under the home
 * directory of the operating-system user running the JustSimple process.
 *
 * <p>The file is intentionally a top-level map so other JustSimple defaults can be
...
final class JwtSecretStore {
    ...
    private static final String SETTINGS_DIR = ".justsimple";
    ...
    private static Map<String, Object> readSettings(Path target) throws IOException {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to parse JustSimple settings JSON: " + target, e);
        }

        if (!(decoded instanceof Map)) {
            throw new IllegalStateException("JustSimple settings JSON must contain a top-level object: " + target);
        }

        Map<String, Object> settings = new LinkedHashMap<>();
        for (Map.Entry<?, ?> entry : ((Map<?, ?>) decoded).entrySet()) {
            if (!(entry.getKey() instanceof String)) {
                throw new IllegalStateException("JustSimple settings JSON contains a non-string key: " + target);
            }
    ...
    private static String readSecret(Map<String, Object> settings) {
        ...
        if (!(value instanceof String)) {
            throw new IllegalStateException("JustSimple setting '" + SETTINGS_KEY + "' must be a string");
        }

        String secret = (String) value;
        if (secret.trim().length() == 0) {
            throw new IllegalStateException("JustSimple setting '" + SETTINGS_KEY + "' must not be empty");
        }

        try {
            Keys.hmacShaKeyFor(Decoders.BASE64.decode(secret));
        } catch (RuntimeException e) {
            throw new IllegalStateException("JustSimple setting '" + SETTINGS_KEY + "' must be a valid Base64 HMAC key");
        }
        return secret;
    }

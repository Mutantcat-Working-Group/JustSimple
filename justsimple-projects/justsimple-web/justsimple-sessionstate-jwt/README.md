...
server.session:
  ...
  state:
    jwt:
      ...
      #secret: "your-base64-secret" #密钥（使用 JwtUtils.createKey() 生成）；（可不配，默认保存到运行 JustSimple 进程的操作系统用户 home 目录下的 ~/.justsimple/settings.json）
...
如果不配置 `server.session.state.jwt.secret`，JustSimple 会首次启动时生成随机密钥，并将其保存到运行 JustSimple 进程的操作系统用户 home 目录下的 `~/.justsimple/settings.json`。该文件使用 Map 结构保存设置，例如：

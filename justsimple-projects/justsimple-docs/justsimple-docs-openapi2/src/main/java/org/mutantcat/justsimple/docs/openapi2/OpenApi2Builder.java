 ...
 */
package org.mutantcat.justsimple.docs.openapi2;
...
import io.swagger.justsimple.annotation.ApiNoAuthorize;
import io.swagger.justsimple.annotation.ApiRes;
import io.swagger.justsimple.annotation.ApiResProperty;
...
import org.mutantcat.justsimple.core.route.VersionedTarget;
import org.mutantcat.justsimple.core.util.EgggUtil;
import org.mutantcat.justsimple.core.util.NameUtil;
import org.mutantcat.justsimple.core.util.PathUtil;
import org.mutantcat.justsimple.core.wrap.*;
import org.mutantcat.justsimple.docs.ApiEnum;
import org.mutantcat.justsimple.docs.DocDocket;
import org.mutantcat.justsimple.docs.exception.DocException;
import org.mutantcat.justsimple.docs.models.ApiContact;
import org.mutantcat.justsimple.docs.models.ApiLicense;
import org.mutantcat.justsimple.docs.models.ApiScheme;
import org.mutantcat.justsimple.docs.openapi2.common.Constants;
import org.mutantcat.justsimple.docs.openapi2.impl.ActionHolder;
import org.mutantcat.justsimple.docs.openapi2.impl.BuilderHelper;
import org.mutantcat.justsimple.docs.openapi2.impl.ParamHolder;
import org.mutantcat.justsimple.docs.openapi2.wrap.ApiImplicitParamImpl;
...
public class OpenApi2Builder {
    ...
    private Map<Class<?>, List<ActionHolder>> getApiAction() {
        ...
        //@since 3.0
        Collection<Routing<Handler>> routingCollection = JustSimple.app().router().findAll();
    ...
    private void parseAction(List<ActionHolder> actionHolders) {
        for (ActionHolder actionHolder : actionHolders) {
            ...
            //支持 context-path
            pathKey = PathUtil.mergePath(JustSimple.cfg().serverContextPath(), pathKey);
        ...
        if (clazz.isInterface()) {
            for (MethodEggg me : ce.getOwnMethodEgggs()) {
                if (me.getParamCount() == 0 && me.getName().startsWith("get")) {
                    ...
                    String propName = NameUtil.getFieldName(me.getName());

                    if (propertyList.containsKey(propName)) {
                        continue;
                    }

                    ApiModelProperty propAnno = me.getMethod().getAnnotation(ApiModelProperty.class);

                    parseSwaggerModelProperty(type, modelName, propertyList, propName, me.getReturnTypeEggg(), propAnno);
                }
            }
        } else {
            for (FieldEggg fe : ce.getAllFieldEgggs()) {
                ...
                String propName = fe.getName();

                ApiModelProperty propAnno = fe.getField().getAnnotation(ApiModelProperty.class);

                parseSwaggerModelProperty(type, modelName, propertyList, propName, fe.getTypeEggg(), propAnno);
            }
        }
    ...
    private void parseSwaggerModelProperty(Type type, String modelName, Map<String, Property> propertyList, String propName, TypeEggg propTypeEggg, ApiModelProperty propAnno) {
        // 隐藏的跳过
        ...
        // List<Class> 类型
        if (Collection.class.isAssignableFrom(propTypeEggg.getType())) {
            // 如果是List类型，得到其Generic的类型
            if (propTypeEggg.isParameterizedType() == false) {
                return;
            }
            ...
            //得到泛型里的class类型对象
            Type itemClazz = propTypeEggg.getActualTypeArguments()[0];
        ...
        if (BuilderHelper.isModel(propTypeEggg.getType())) {
            if (propTypeEggg.getType().equals(type)) {
                //避免出现循环依赖，然后 oom
            ...
            if (propAnno != null) {
                ...
                propPr.setRequired(propAnno.required());
                propPr.setExample(propAnno.example());
                if(FileBase.class.isAssignableFrom(propTypeEggg.getType())){
                    // 文件类型固定为file
                    propPr.setType("file");
                }else{
                    propPr.setType(Utils.isBlank(propAnno.dataType()) ? propTypeEggg.getType().getSimpleName().toLowerCase() : propAnno.dataType());
                }
            } else {
                propPr.setType(propTypeEggg.getType().getSimpleName().toLowerCase());
            }

...
import org.mutantcat.justsimple.docs.integration.properties.DocDocketProperties;
import org.mutantcat.justsimple.docs.integration.properties.DocsProperties;
...
public class DocsPlugin implements Plugin {
    ...
    public static final String JUSTSIMPLE_DOCS = "justsimple.docs";

    @Override
    public void start(AppContext context) throws Throwable {
        final Props docsProps = context.cfg().getProp(JUSTSIMPLE_DOCS);
        ...
        //加载 justsimple.docs.discover.pathPattern
        if (docsProperties.getDiscover().isEnabled()) {
            if (ClassUtil.hasClass(() -> Discovery.class)) {
                ...
                log.warn("JustSimple docs discover: missing justsimple cloud discovery");
            }
        }
    }
}

...
> 链路：客户端 → JustSimple Server 线程（阻塞）→ HttpUtils.execDo(sync) → 服务端
...
> 链路：客户端 → JustSimple Server → HttpUtils.execAsync → OkHttp Dispatcher → 服务端
...
> 链路：客户端 → JustSimple Server 线程（阻塞）→ HttpURLConnection → 服务端
...
> 链路：客户端 → JustSimple Server → HttpUtils.execAsync → 自建线程池（core=0, max=maxRequests, SynchronousQueue, CallerRunsPolicy）→ 服务端
...
> **警告**：JDK 异步线程池使用 `SynchronousQueue + corePoolSize=0`，无线冲缓冲。达到 maxThreads 后触发 `CallerRunsPolicy`，阻塞 JustSimple Server 线程，可能导致雪崩。
...
String body = HttpUtils.http("http://localhost:8080/api/user")
        .bodyOfJson("{\"name\":\"justsimple\",\"age\":1}")
        .post();

// POST 表单
String body = HttpUtils.http("http://localhost:8080/api/login")
        .bodyOfForm("username=justsimple&password=123456")
        .post();
```
...
### 通过服务名调用（需配合 JustSimple Cloud 注册中心）
...
public class DemoApp {
    public static void main(String[] args) {
        JustSimple.start(DemoApp.class, args, app -> {
            // 在首次请求前配置连接池
            HttpConfiguration.setMaxIdleConnections(200);
            HttpConfiguration.setMaxRequests(300);
            HttpConfiguration.setMaxRequestsPerHost(256);
...
public class ApiController {
    ...
    public String sync() {
        return HttpUtils.http("http://localhost:8081/user/info")
                .timeout(5, 30, 10)
                .header("X-Trace-Id", JustSimple.cfg().get("justsimple.app.id"))
                .get();
    }

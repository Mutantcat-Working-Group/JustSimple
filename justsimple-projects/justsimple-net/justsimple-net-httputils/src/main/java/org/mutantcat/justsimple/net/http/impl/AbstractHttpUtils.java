 ...
 */
package org.mutantcat.justsimple.net.http.impl;
...
import org.mutantcat.justsimple.net.http.textstream.TextStreamUtil;
...
import org.mutantcat.justsimple.net.http.*;
...
public abstract class AbstractHttpUtils implements HttpUtils {
    ...
    public Serializer serializer() {
        if (_serializer == null) {
            if (JustSimple.app() != null) {
                _serializer = JustSimple.app().serializers().get(SerializerNames.AT_JSON);
            }

            if (_serializer == null) {
                throw new JustSimpleException("Missing serializer!");
            }
        }
    ...
    /**
     * 用户代理配置（不设置时使用默认 UA：justsimple-http/<version>）
     */
    @Override
    ...
    public String execAsBody(String method) throws HttpException {
        try {
            ...
            throw new JustSimpleException(e);
        }
    }

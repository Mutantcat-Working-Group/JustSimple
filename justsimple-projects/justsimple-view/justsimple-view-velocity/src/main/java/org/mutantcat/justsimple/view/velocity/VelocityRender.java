 ...
 */
package org.mutantcat.justsimple.view.velocity;
...
import org.mutantcat.justsimple.server.util.DebugUtils;
...
public class VelocityRender implements Render {
    ...
    private void engineInit(RuntimeInstance ve) {
        ...
        ve.setProperty(Velocity.ENCODING_DEFAULT, JustSimple.encoding());
        ve.setProperty(Velocity.INPUT_ENCODING, JustSimple.encoding());

        JustSimple.cfg().forEach((k, v) -> {
    ...
    private void forDebug() {
        if (JustSimple.cfg().isDebugMode() == false) {
            return;
        }

        if (JustSimple.cfg().isFilesMode() == false) {
            return;
        }
    ...
    protected void doRender(ModelAndView mv, Context ctx, SupplierEx<OutputStream> outputStream) throws Throwable {
        ...
        if (providerOfDebug != null) {
            try {
                template = providerOfDebug.getTemplate(view, JustSimple.encoding());
        ...
        if (template == null) {
            template = provider.getTemplate(view, JustSimple.encoding());
        }

 ...
 */
package org.mutantcat.justsimple.view.freemarker;
...
public class FreemarkerRender implements Render {
    ...
    protected void doRender(ModelAndView mv, Context ctx, SupplierEx<OutputStream> outputStream) throws Throwable {
        ...
        if (providerOfDebug != null) {
            try {
                template = providerOfDebug.getTemplate(mv.view(), JustSimple.encoding());
        ...
        if (template == null) {
            template = provider.getTemplate(mv.view(), JustSimple.encoding());
        }

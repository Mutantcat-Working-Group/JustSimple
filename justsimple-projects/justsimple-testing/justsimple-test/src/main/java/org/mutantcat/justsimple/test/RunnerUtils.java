 ...
 */
package org.mutantcat.justsimple.test;

import org.mockito.Mock;
import org.mockito.Mockito;
import org.mutantcat.justsimple.*;
...
import org.mutantcat.justsimple.test.aot.JustSimpleAotTestProcessor;
import org.mutantcat.justsimple.test.data.RollbackInterceptor;
...
public class RunnerUtils {
    ...
    private static Class<?> getMainClz(JustSimpleTest anno, Class<?> klass) {
        if (anno == null) {
            return klass;
        }
    ...
    public static AppContext initRunner(Class<?> klass) throws Throwable {
        ...
        JustSimpleTest anno = klass.getAnnotation(JustSimpleTest.class);
        ...
        if (anno != null) {
            ...
            try {
                ...
                if (anno.isAot()) {
                    new JustSimpleAotTestProcessor(mainClz).process(appContext);
                }
    ...
    private static AppContext startDo(Class<?> mainClz, List<String> argsAry, Class<?> klass, boolean enableHttp, boolean enableWebSocket) throws Throwable {
        ...
        if (mainClz == klass) {
            ...
            SimpleJustSimpleApp testApp = new SimpleJustSimpleApp(mainClz, args);
            testApp.start(x -> {
            ...
            if (main != null && Modifier.isStatic(main.getModifiers())) {
                ...
                if (JustSimple.app() != null) {
                    return JustSimple.context();
                } else {
                    //如果不是 justsimple 应用，则启动一个 justsimple
                    SimpleJustSimpleApp testApp = new SimpleJustSimpleApp(mainClz, args);
                    testApp.start(x -> {
                        //默认关闭 http（避免与已经存在的服务端口冲突）
    ...
    private static void initDo(Class<?> klass, JustSimpleApp app) throws Throwable {
